document.addEventListener('DOMContentLoaded', () => {
    ...
    function initializeMaze() {
        ...
        const numRows = mazeStringArray.length; // Use length of layout array

        for (let row = 0; row < numRows; row++) {
            const rowContainer = document.createElement('div');
            rowContainer.classList.add('maze-row');

            const numCols = mazeStringArray[row].length; // Adjust to use length of each row
    ...
    function calculateNewPosition(key, position) {
        ...
        if (key === 'arrowup' && newRow > 0) {
            newRow--;
        } else if (key === 'arrowdown' && newRow < mazeConfigurations[mazeIndex].layout.length - 1) {
            newRow++;
